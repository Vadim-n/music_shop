...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            amount(newVal) {
                let old = newVal;
                if (this.can_be_watch) {
                    if (newVal) {
                        newVal = newVal.replace('ю', ',');
                        newVal = newVal.replace('б', this.decimalSeparatorSymbol);
                        newVal = newVal.replace('/', this.decimalSeparatorSymbol);
                        newVal = newVal.replace('?', this.decimalSeparatorSymbol);
                        if (this.decimalSeparatorSymbol === ',') {
                        ...
                        if (newVal !== old) {
                            this.can_be_watch = false;
                            this.amount = newVal;
                        }
                    }
                }
                else {
                    this.can_be_watch = true;
                }
            }
        },
        ...
        methods: {
            ...
            unformat(value) {
                ...
                if (this.decimalSeparatorSymbol === ',') {
                    toUnformat = toUnformat.toString().replace('ю', ',');
                    toUnformat = toUnformat.toString().replace('б', ',');
                    toUnformat = toUnformat.toString().replace('/', ',');
                    toUnformat = toUnformat.toString().replace('?', ',');
                    toUnformat = toUnformat.toString().replace('.', ',');
                }
                if (this.decimalSeparatorSymbol === '.') {
                ...
                if (this.decimalSeparatorSymbol !== '') {
                    ...
                    return accounting.unformat(toUnformat);
                }
            },
            emitChange (){
                this.$emit('change');
